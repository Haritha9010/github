...
from App.forms import RegForm,ChpwdForm,WorkerForm,CustomiseForm,CategoryForm,ProductForm,RoleR,RoleUp,UpPrf,Procustom,CancelForm,PlaceorderForm
...
def checkout(request):
	c=Cart.objects.filter(user_id=request.user.id)
	d=Category.objects.all()
	po=PlaceorderForm(request.POST)
	if request.method=="POST":
		if po.is_valid():
...
def sort(request,id):
	...
	for i in p:
		l.append(i.price)
		l.sort()
	return render(request,'html/product.html',{'pr':l})